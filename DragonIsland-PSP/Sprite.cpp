...
bool Sprite::setAnimationState(string state) {
    for (int i = 0; i < (int)mFrameInfo.size(); i++) {
...
void Sprite::draw(Point* cam)
{
    ...
    if (!mRotate) {
        if (!mFlicker) {
			...
			mSpriteSheet->drawFrame(mX - cam->x, mY + 1 - cam->y,  mFrameIndex, mDirection);	
        } else {
            if (mFrameTimer % 5 == 0) {
                //g.drawImage(mSpriteSheet.getFrame(mFrameIndex, mDirection), mX - cam.x, mY + 1 - cam.y, null);
				mSpriteSheet->drawFrame(mX - cam->x, mY + 1 - cam->y,  mFrameIndex, mDirection);
    ...
    {
        if (mAngle < 360) {
            mAngle += 10;// mAnimationSpeed; //rotate sprite at the animation speed
        } else {
            mAngle = 0;
        }
        if (getType() >= 20) 
        {
			mSpriteSheet->drawFrame(mX - cam->x, mY + 1 - cam->y,  mFrameIndex, mDirection, mAngle);
            //g2d.drawImage(mSpriteSheet.getFrame(mFrameIndex, mDirection), at, null);
        } else //draw powerup
        {
			mSpriteSheet->drawFrame(mX - cam->x, mY + 1 - cam->y,  mFrameIndex, 'l', mAngle);
    ...
    {
		oslDrawRect(getLeft() - cam->x, getUp() - cam->y, getLeft() - cam->x + getWidth(), getUp() - cam->y + getHeight(), RGB(255, 0, 0));
 ...
 */
void Sprite::drawXY(Point* cam, char dir, int x, int y) {
	mSpriteSheet->drawFrame(x - cam->x, y - cam->y, mFrameIndex, dir);
}
 ...
 */
OSL_IMAGE* Sprite::getFrame(char direction) {
 ...
 * @return The image data for the frame of animation
 */
OSL_IMAGE* Sprite::getFrame(int frameIndex, char direction) {
 ...
 * @return The image data for the frame of animation
 */
OSL_IMAGE* Sprite::getFrame(int frameIndex, char direction, int height) {
    //try {
/*
...
Point* Sprite::getCenter() {
	delete center;
	center = new Point(getLeft() + getWidth() / 2, getUp() + getHeight() / 2);
    return center;
}
...
/**
 * Gets the falling speed of the sprite
 * @return The change in y coordinate when the sprite is falling
 */
int Sprite::getFallSpeed() {
    return mFallSpeed;
}
...
/**
 * Sets the jump speed
 * @param jumpSpeed The amount to decrease the y coordinate when falling
 */
