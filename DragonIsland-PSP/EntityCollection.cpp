 ...
 */
OSL_IMAGE* EntityCollection::getEntity(int index, char direction) {
 ...
 */
void EntityCollection::draw(Map* map, Point* cam) {
    ...
    for (int x = cam->x / 16 - 10; x < (cam->x + 512) / 16; x++) {
        for (int y = cam->y / 16; y < (cam->y + 300) / 16; y++) {		
            ...
            if (Map::isEnlargeBlock(mTileInfo->tile1, mTileInfo->tile2)) {
                mEntities[10]->drawXY(cam, 'l', x * 16, (y - 1) * 16);
            } else if (Map::isFireBlock(mTileInfo->tile1, mTileInfo->tile2)) {
                mEntities[11]->drawXY(cam, 'l', x * 16, (y - 1) * 16);
            } else if (Map::isInvincibleBlock(mTileInfo->tile1, mTileInfo->tile2)) {
                mEntities[12]->drawXY(cam, 'l', x * 16, (y - 1) * 16);
            } else if (Map::isExtraLifeBlock(mTileInfo->tile1, mTileInfo->tile2)) {
                mEntities[13]->drawXY(cam, 'l', x * 16, (y - 1) * 16);
            } else if (Map::isVineBlock(mTileInfo->tile1, mTileInfo->tile2)) {
                mEntities[14]->drawXY(cam, 'l', x * 16, (y - 1) * 16);
            } else {
                if (mTileInfo->spriteTile != 0 && mTileInfo->spriteDraw == 1) //sprite
                {
                    mEntities[mTileInfo->spriteTile]->drawXY(cam, 'l', x * 16, y * 16);
                }
            }
        }
    }
