...
Entity::Entity(int index, char direction, int x, int y) : Sprite(index, direction, x, y) {
	...
	char path[100];
    sprintf (path, "res/spr/%d", getIndex());	
 ...
 */
void Entity::drawImage(OSL_IMAGE* img, int x, int y)
{
...
void Entity::draw(Point* cam) {
    ...
    if (getType() == 14) {
		...
		int height = (mStartY - getDown() + 15) / 16;
		
		OSL_IMAGE* img = getFrame(0, 'l');
		drawImage(img, getX() - cam->x, getDown() - cam->y);
		
		for(int i = 0; i < height; i++)		
			drawImage(img, getX() - cam->x, (getDown() - cam->y) + ((i+1)* 16));
			
		oslDeleteImage(img);

        //draw bounds including growing vine
        if (Settings::DrawBounds) {
			Rectangle* vb = getVineBounds();
			oslDrawRect(vb->x - cam->x, vb->y - cam->y, vb->x +  vb->w - cam->x, vb->y + vb->h - cam->y, RGB(255, 0, 0));
			delete vb;
        }
    }
...
Rectangle* Entity::getVineBounds() {
	return new Rectangle(getX() + 6, getDown() + 4 - 16, 1, (mStartY + 16) - (getDown() + 4 - 16));
}
...
/**
 * Deallocates memory by destroying the Entity
 */
Entity::~Entity() {
