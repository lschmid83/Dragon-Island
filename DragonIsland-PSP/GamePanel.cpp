...
bool GamePanel::loadLevel(string gameFolder, int world, int level, int area) {
	...
	char path[100];
	sprintf(path, "res/lvl/%s/%d.%d.%d.lvl", gameFolder.c_str(), world, level, area);	
...
/**
 * Closes the level and deallocates memory
 */
...
void GamePanel::newGame() {
	...
	char tmp[100];
	sprintf(tmp, "res/lvl/%s/0.0.1.ls", SaveFile::GameName[SaveFile::GameFolder].c_str());	
	ifstream file(tmp); 
...
void GamePanel::advanceLevel() {
    ...
    int pSize = mPlayer->getSize();
    int pInvincible = mPlayer->getInvincible();
    int pLives = mPlayer->getLives();
    int pCoins = mPlayer->getCoins();
    int pScore =  mPlayer->getScore();
    int endArea = mLevelHeader->EndArea;
    ...
    {	
		...
		if (mOnBonus) //restart player at bonus position
        {
            ...
            //Settings::State = "level open";
            mPlayer = new Player(Settings::MainCharacter, pSize, pInvincible, pLives, pCoins, pScore, mLevelInfo->getHeader()->TimeLimit, state, mLevelHeader->BonusX * 16, mLevelHeader->BonusY * 16);
        ...
        { 
			if(endArea == 1)
				...
				Settings::Paused = false;	
            mPlayer = new Player(Settings::MainCharacter, pSize, pInvincible, pLives, pCoins, pScore, mLevelInfo->getHeader()->TimeLimit, mLevelHeader->StartState, mLevelHeader->StartX * 16, mLevelHeader->StartY * 16);
            //loadScript();
            //updateSaveGameFile();
        }
    } else {	
...
void GamePanel::setMusic(int file) {
    ...
    //if (file != 0) {
		char path[100];
		sprintf(path, "res/snd/%d.mp3", file);	
...
void GamePanel::playbackMovement(string gameFolder, int world, int level, int area) {
 	char path[100];
	sprintf(path, "res/rec/%s.%d.%d.%d.rec", gameFolder.c_str(), world, level, area);  
...
void GamePanel::loadScript() {
	...
	char path[100];
	sprintf(path, "res/lvl/%s/%d.%d.%d.ls", SaveFile::GameName[SaveFile::GameFolder].c_str(), mLevelInfo->getHeader()->World, mLevelInfo->getHeader()->Level, mLevelInfo->getHeader()->Area);  	
	ifstream input(path); 
	if(input.good())
	{
 ...
 */
void GamePanel::paintComponent() {

    if (Settings::LevelEditor) //level editor
    {
        main::mEditorPanel->paintComponent();
    } 
	else  //main game
    {
        if (!Settings::Paused  && Settings::State != "debug") {
            ...
            mCamera = mBackground->draw(mCamera);

            //draw objects 
            mMapTileset->drawMap(mLevelMap, mCamera);
                    ...
                    for (int i = 0; i < mPlaybackScript.size(); i++) {
                        if ((mScriptTimer == mPlaybackScript[i].time) || mPlaybackScript[i].timer > 0) 
                        {
                            ...
                            if (sc.command =="Earthquake") {
                                ...
                                mUserInterface->drawSpeech(sc.x, sc.y, sc.width, sc.height, sc.text, sc.direction);
...
*/
            ...
            for (int i = 0; i < (int)mEntities.size(); i++) {
                if (mEntities[i]->getX() > mCamera->x - 250  && mEntities[i]->getX() < mCamera->x + 580) //enemy is on screen
                {
                    mEntities[i]->detectMapCollision(mLevelMap); //detect map collisions
                    mEntities[i]->draw(mCamera); //paint / move enemy
                }
		        if (mEntities[i]->getAnimationState() == "dead") //remove dead sprite except fireball
		        {	
					delete mEntities[i];
            ...
            //draw objects 
            mMapTileset->drawVineBlocks(mLevelMap, mCamera);

            //draw map object explosions
            mMapTileset->drawExplosions(mLevelMap, mCamera);

            //detect collisions between player and entities
            mPlayer->detectEntityCollision(mEntities);

            //draw player
            mPlayer->draw(mCamera);

            if (mPlayer->getPipeState() > 1) //redraw map objects
            {
                mMapTileset->drawMap(mLevelMap, mCamera); //draw pipe over player
            }

            //draw castle extension over player
			mMapTileset->drawImage(mMapTileset->getFrame(2, 0, 0), mCastlePosition->x - mCamera->x, mCastlePosition->y - mCamera->y );
            ...
            if (mPlayer->getUp() > mLevelMap->getHeight() + mPlayer->getHeight() + 20) {
    ...
    if (Settings::State == "game") {
        ...
        {
            mUserInterface->drawStatus(50, 15, mPlayer->getLives(), mPlayer->getCoins(), mPlayer->getScore(), mPlayer->getTime());
        }
    } else {
		if(Settings::State != "debug")     
			mUserInterface->draw(); //draw screen menu
    }
    ...
    if (Settings::DrawBounds && Settings::State == "game") {
		...
		mUserInterface->drawString(str, 15, 15);

        sprintf (str, "Tileset: %d-%d-%d", mLevelInfo->getHeader()->Tileset0, mLevelInfo->getHeader()->Tileset16, mLevelInfo->getHeader()->Tileset32);	
		mUserInterface->drawString(str, 15, 30);

        sprintf (str, "Background: %d-%d-%d",  mLevelInfo->getHeader()->Bg[0],  mLevelInfo->getHeader()->Bg[1], mLevelInfo->getHeader()->Bg[2]);	
		mUserInterface->drawString(str, 15, 45);

        sprintf (str, "Music: %d", mLevelInfo->getHeader()->Music);	
		mUserInterface->drawString(str, 15, 60);

        sprintf (str, "Map Width: %d px", mLevelInfo->getHeader()->Width);	
		mUserInterface->drawString(str, 15, 75);

        sprintf (str, "Map Height: %d px", mLevelInfo->getHeader()->Height);	
		mUserInterface->drawString(str, 15, 90);

        sprintf (str, "Character: %d", Settings::MainCharacter);	
		mUserInterface->drawString(str, 150, 15);

        //Rectangle* pb = mPlayer->getBounds();
        sprintf (str, "Coordinates: x=%d y=%d", mPlayer->getCenter()->x / 16,  mPlayer->getY() / 16);
		mUserInterface->drawString(str, 150, 30);
		//delete pb;

        sprintf (str, "Bonus: x=%d y=%d",  mLevelInfo->getHeader()->BonusX,  mLevelInfo->getHeader()->BonusY);	
		mUserInterface->drawString(str, 150, 45);
/*
		Rectangle* eb = mEntities[0]->getBounds();
		sprintf (str, "Enemy: x=%d y=%d", eb->x, eb->y);	
		mUserInterface->drawString(str, 150, 45);
		delete eb;
*/
        sprintf (str, "Animation: %s", mPlayer->getAnimationState().c_str());	
		mUserInterface->drawString(str, 150, 60);

        sprintf (str, "Time: %d",  mPlayer->getTime());	
		mUserInterface->drawString(str, 150, 75);

        sprintf (str, "Lives: %d", mPlayer->getLives());	
		mUserInterface->drawString(str, 150, 90);
    }

    if (mUserInterface->showTransition) {
        mUserInterface->drawScreenTransition();
    }
...
*/
void GamePanel::keyPressed(OSL_CONTROLLER e) {

	if (Settings::State == "title") {
        if (e.pressed.up) {
            mKeyBuffer += 'u';
        } else if (e.pressed.down) {
            mKeyBuffer += 'd';
        } else if (e.pressed.left) {
            mKeyBuffer += 'l';
        } else if (e.pressed.right) {
            mKeyBuffer += 'r';
		} else if (e.pressed.square) {
            mKeyBuffer += 'b';
		} else if (e.pressed.cross) {
            mKeyBuffer += 'a';
        }
    ...
    if(e.pressed.select)
    	Settings::DrawBounds = !Settings::DrawBounds;
    
    
	if (e.pressed.R && Settings::LevelSelect && Settings::State == "game") {
        advanceLevel();
    }
    ...
    if (Settings::LevelEditor)
		main::mEditorPanel->keyPressed(*osl_keys);	

    if (e.pressed.start && !mUserInterface->showTransition) {
 ...
 * @param e event which indicates that a keystroke occurred in a component
*/
void GamePanel::keyReleased(OSL_CONTROLLER e) {
	setControl(e, false);
}
 ...
 */
void GamePanel::setControl(OSL_CONTROLLER e, bool pressed) {
	
	if(e.held.cross)
		mPlayer->mControls->mJumpPressed = true;
	else
	{
		mPlayer->mControls->mJumpReleased = true;
		mPlayer->mControls->mJumpPressed = false;
	}

	if(e.held.up)
		mPlayer->mControls->mUpPressed = true;
	else
		mPlayer->mControls->mUpPressed = false;

	if(e.held.down)
		mPlayer->mControls->mDownPressed = true;
	else
		mPlayer->mControls->mDownPressed = false;

	if(e.held.left)
		mPlayer->mControls->mLeftPressed = true;
	else
		mPlayer->mControls->mLeftPressed = false;

	if(e.held.right)
		mPlayer->mControls->mRightPressed = true;
	else
		mPlayer->mControls->mRightPressed = false;

	if(e.held.square)
		mPlayer->mControls->mRunPressed = true;
	else
	{
		mPlayer->mControls->mRunPressed = false;
 ...
 */
void GamePanel::mouseMoved(OSL_CONTROLLER e) {
 ...
 * @param e event which indicates that a mouse action occurred in a component
 */
void GamePanel::mouseDragged(OSL_CONTROLLER e) {
    if (Settings::State == "level editor") {
 ...
 * @param e event which indicates that a mouse action occurred in a component
 */
void GamePanel::mouseReleased(OSL_CONTROLLER e) {
    if (Settings::State == "level editor") {
 ...
 * @param e event which indicates that a mouse action occurred in a component
 */
void GamePanel::mousePressed(OSL_CONTROLLER e) {
    if (Settings::State == "level editor") {
 ...
 */
void GamePanel::mouseWheelMoved(OSL_CONTROLLER e) {
    if (Settings::State == "level editor") {
