...
TileDescription* Level::getTile(int index)
{
	return mTiles[index];
}
 ...
 */
Map* Level::toMap(Map* map) {

    for (int i = 0; i < (int)mEntities.size(); i++) {
		if(mEntities[i]->X >=0 && mEntities[i]->Y >=0)
			map->setEntityTile(i, mEntities[i]);
    }

    for (int i = 0; i < (int)mTiles.size(); i++) {
		if(mTiles[i]->X >=0 && mTiles[i]->Y >=0)
...
void Level::newLevel() {
    ...
    mHeader->Bg[1] = 1;
    mHeader->Bg[2] = 1;   
    mHeader->BgColor = new Color(0,0,0);
 	mHeader->BgSpeed[0] = 0;
	mHeader->BgSpeed[1] = 0;
	mHeader->BgSpeed[2] = 1;   	
	mHeader->BgAlign = 0;
...
vector<string> Level::getTileStringDescription(int tileIndex, int tilesetIndex) {
    ...
    for (int i = 0; i < (int)mTileDefinitions.size(); i++) //array of definitions
    {
...
vector<string> Level::getEntityStringDescription(int tile) {
    ...
    for (int i = 0; i < (int)mEntitiyDefinitions.size(); i++) {
    	vector<string> desc;
...
void Level::initDefinitions() {
	...
	char path[100];    
	string line;    
    sprintf (path, "res/obj/block/%d/tileset.ini", mHeader->Tileset0);	    
	...
	//tileset 2
    sprintf (path, "res/obj/terrain/%d/tileset.ini", mHeader->Tileset0);	    
	...
	//tileset 3
    sprintf (path, "res/obj/scenery/%d/tileset.ini", mHeader->Tileset0);	    
   	file.open(path); 
	...
	//sprite
    sprintf (path, "res/spr/sprite.ini");	    
    file.open(path); 
