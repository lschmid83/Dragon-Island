...
Player::Player(int character, int size, int invincible,
	...
	char path[100];
    sprintf (path, "mnt/sd/Dragon Island/chr/%d/%d", getIndex(), size);	
    ...
    //mControls = new Control();
    sprintf (path, "mnt/sd/Dragon Island/chr/%d/3.png", getIndex());	
 ...
 */    
void Player::draw(SDL_Surface* g, Point* cam) {

    Sprite::draw(g, cam); //draw the sprite
    ...
    for (int i = 0; i < mDrawPoints.size(); i++) {
		...
		if (mDrawPoints[i].timer < 30)
        	mPointFont->drawString(g, 0, convertInt(mDrawPoints[i].value), mDrawPoints[i].x - cam->x, mDrawPoints[i].y - cam->y);
    ...
    if (getAnimationState() != "die" && mFlagState == 0 && mPipeState < 2) {
        ...
        if (mInvincible == 1) //invincibility powerup
        {
            ...
            if (getDirection() == 'r') {
				mStars->drawFrame(g, getCenter()->x - 31 - cam->x, getUp() - 10 - cam->y, mInvincibleFrame, 'r');
            } else {
				mStars->drawFrame(g, getCenter()->x - 19 - cam->x, getUp() - 10 - cam->y, mInvincibleFrame, 'r');
            }
...
void Player::setMovement() {
    ...
    if (getDown() == getMaxY() && getAnimationState() != "climb") //on ground level
    {
        ...
        //jump button is not pressed and player has landed
...
bool Player::detectEntityCollision(vector<Entity*> entities) {
    ...
    for (int a = 0; a < entities.size(); a++) {
        ...
        if (e->getType() == 1) //fireball
        {
            ...
            for (int b = 0; b < entities.size(); b++) {
                ...
                if (a != b && e->getBounds()->intersects(entities[b]->getBounds())) {
                    ...
                    {
                        if (!e->getRotate() && entities[b]->getType() >= 20 && entities[b]->getType() < 50) {
                            //remove fireball
                            ...
                            entities[b]->setSpeed(1);
                            entities[b]->setJumpSpeed(1);
                            entities[b]->setFallSpeed(2);
                            if (getDirection() == 'r') {
                                entities[b]->setDirection('r');
                            } else {
                                entities[b]->setDirection('l');
                            }
                            entities[b]->setMinY(entities[b]->getUp() + 6);
        ...
        if (getBounds()->intersects(e->getVineBounds()) && e->getType() == 14) //player intersects growing vine entity
        {
            ...
            if (mControls.mUpPressed && (getAnimationState() == "jump" || getAnimationState() == "stand")) {
                setAnimationState("climb"); //set climb animation
                setJump(false);
                if (getSpeed() > 1) {
                    setSpeed(0); //important stop player moving
                }
                setClimb(true); //set state to climb stop sprite from moving down due to gravity
            }
        } else {
            ...
            //check other vines for collision slow needs fixing!!
            for (int c = 0; c < entities.size(); c++) {
                Entity* d = entities[c];
                if (getBounds()->intersects(d->getVineBounds()) && d->getType() == 14 && getAnimationState() == "climb") {
                    setClimb(true);
                    setJump(false);
                    if (getSpeed() > 1) {
                        setSpeed(0);
...
bool Player::detectMapCollision(Map* map) {
    ...
    if (getCenter()->x / 16 == map->getBonusX() && getY()/ 16 == map->getBonusY()) //bonus level
    {
        exitX = map->getBonusX();
        exitY = map->getBonusY();
