		...
		struct dirent * dp;
	   	...
	   	while(dp = readdir(dir)){		  
			const string str = name + dp->d_name;
		  	if (Exists(str + "/")){
				if (recurse) GetAllFiles(str + "/", res, true, ext);
		  	} else {
				//struct stat statbuf;
				string tmp = name + "/" + dp->d_name;	
			 	//if(stat(tmp.c_str(), &statbuf) != -1) //http://www.quora.com/How-do-I-check-if-a-directory-exists-in-Linux-using-C++/answers/390986
			 	//{
				//	 if (!S_ISDIR(statbuf.st_mode)) { //is file
						if(tmp.substr(tmp.size()-3, tmp.size()) == ext) 
						{
...
void SaveFile::refreshLevelList() {
    if (GameName[0] != "Empty") {
        LevelName = SaveFile::getFileList("mnt/sd/Dragon Island/lvl/" + GameName[GameFolder] + "/", "lvl");
    }
}
...
void SaveFile::refreshGameList() {
    GameName = SaveFile::getFolderList("mnt/sd/Dragon Island/lvl/");
    if (GameName.size() == 0) {
