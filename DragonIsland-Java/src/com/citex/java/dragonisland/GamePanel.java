...
public class GamePanel extends JPanel implements KeyListener, MouseListener,
    ...
    private RandomAccessFile mRecordScript;
	...
	public static int mLevelLoadingTimer;
    ...
    public void newGame() {
        ...
        try //intro
        {
            RandomAccessFile f = new RandomAccessFile("res/lvl/" + SaveFile.mGameName.get(SaveFile.mGameNumber) + "/" + 0 + "." + 0 + "." + 1 + ".ls", "r");
            f.close();
            Settings.State = "game";
            loadLevel(SaveFile.mGameName.get(SaveFile.mGameNumber), 0, 0, 1);
        } catch (Exception e) {
            Settings.State = "level open";
            Settings.Paused = true; //stop playback until game starts
            // System.err.println("Error: Cannot find the new game intro file.");
            loadLevel(SaveFile.mGameName.get(SaveFile.mGameNumber), 1, 1, 1);           	
        }
    ...
    public void advanceLevel() {
        ...
        if (loadLevel(SaveFile.mGameName.get(SaveFile.mGameNumber), mLevelHeader.endWorld, mLevelHeader.endLevel, mLevelHeader.endArea)) {
    ...
    public void advanceBonus() {
        if (loadLevel(SaveFile.mGameName.get(SaveFile.mGameNumber), mLevelHeader.world, mLevelHeader.level, 0)) {
            if (mLevelHeader.timeLimit == 0) //no timelimit set
            {
    ...
    public void resetLevel() {
        ...
        int area = mLevelHeader.area;
        if(mLevelHeader.area == 0)
        {
        	area = 1;
        }
        
        if(loadLevel(SaveFile.mGameName.get(SaveFile.mGameNumber), mLevelHeader.world, mLevelHeader.level, area))
        {
    ...
    public void setMusic(int file) {
        if (file != 0 && Settings.Music) {
            String path = System.getProperty("user.dir") + "\\res\\snd\\" + file + ".mp3"; //".mp3";
            if (mMusic != null) {
                Music.closeMusic();
            }
    ...
    public void recordMovement(String gameFolder, int world, int level, int area) {
        ...
        try {
            mRecordScript = new RandomAccessFile(fileName, "rw");
            mScriptTimer = 0;
            mScriptMode = 1;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void loadScript() {
        try {
            RandomAccessFile input = new RandomAccessFile("res/lvl/" + SaveFile.mGameName.get(SaveFile.mGameNumber) + "/" + mLevelInfo.getHeader().world + "." + mLevelInfo.getHeader().level + "." + mLevelInfo.getHeader().area + ".ls", "r");
            ...
            playbackMovement(SaveFile.mGameName.get(SaveFile.mGameNumber), mLevelInfo.getHeader().world, mLevelInfo.getHeader().level, mLevelInfo.getHeader().area);
    ...
    public void paintComponent(Graphics g) {
        ...
        else if (Settings.State.equals("license")) //license screen
		{
        ...
        else if(Settings.State.equals("error")) //error cannot find resources
		{
			...
			back_buffer_graphics.drawString("Re-download Dragon Island from www.citexsoftware.co.uk/dragonisland", 8, 47);
		}
        else //main game
        {
            if (!Settings.Paused) {
                ...
                if ((mRecordScript != null && mScriptMode > 0) && !mUserInterface.showTransition) {
                    ...
                    if (mScriptMode == 1) //record
                    {
                        try {
                            //write player movement to file
                            mScriptTimer++;
                            mRecordScript.writeInt(mPlayer.getX());
                            mRecordScript.writeInt(mPlayer.getY());
                            mRecordScript.writeChar(mPlayer.getDirection());
                            mRecordScript.writeInt(mPlayer.getAnimationStateIndex());
                        } catch (Exception e) //error writing to .rec file
                        {
                            e.printStackTrace();
                            mScriptMode = 0;
                            try {
                    ...
                    } else if (mScriptMode > 1 && mScriptMode < 4) //play
                    {
                        try {
                            //set player movement
                            mScriptTimer++; //advance capture / playback  timer
                            mPlayer.setX(mRecordScript.readInt());
                            mPlayer.setY(mRecordScript.readInt());
                            mPlayer.setDirection(mRecordScript.readChar());
                            mPlayer.setAnimationStateIndex(mRecordScript.readInt());
                        ...
                        for (int i = 0; i < mPlaybackScript.size(); i++) {
                            if ((mScriptTimer == mPlaybackScript.get(i).time) || mPlaybackScript.get(i).timer > 0) //if script command == capture timer
                            {
                                ...
                                if (sc.command.equals("Earthquake")) {
                                    ...
                                    addEntity(Integer.parseInt(sc.text), sc.direction, sc.x, sc.y);
        ...
        if(!Settings.State.equals("error")) 
        {
	        //draw status bar
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (Settings.DebugMode) {
            if (e.getKeyCode() == KeyEvent.VK_R) {
                if (Settings.State.equals("game")) {
                    recordMovement(SaveFile.mGameName.get(SaveFile.mGameNumber), mLevelInfo.getHeader().world, mLevelInfo.getHeader().level, mLevelInfo.getHeader().area);
                }
            } else if (e.getKeyCode() == KeyEvent.VK_P) {
                if (Settings.State.equals("game") && mScriptMode < 3) {
                    mScriptTimer = 0;
                    mScriptMode = 2;
                    playbackMovement(SaveFile.mGameName.get(SaveFile.mGameNumber), mLevelInfo.getHeader().world, mLevelInfo.getHeader().level, mLevelInfo.getHeader().area);
                }
