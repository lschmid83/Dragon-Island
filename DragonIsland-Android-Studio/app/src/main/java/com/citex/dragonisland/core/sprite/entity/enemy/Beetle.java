...
public class Beetle extends Sprite implements IEntity {
     ...
     */
    public Beetle(Main main, Object g, String path) throws NumberFormatException, IOException {
    	...
    	// Initialise velocity.
    	setVelocity(new Point(0.06f, 0.2f));
    ...
    /**
   	...
   	public void move(float dt, Point cam) {
        ...
        if(getAnimationState().equals("hit") || getAnimationState().equals("kick")) {
        	...
        	// Wait while the returns to normal.
        	if(mHitTimer < 3500) {
        		mHitTimer += dt;
        	} else {
	...
	public void detectMapCollision(Map map) {
        ...
        // Gap.
        if(!getAnimationState().equals("kick") && getDown() == getMaxMove().y) {
        ...
        if (getBounds().intersects(player.getBounds())) {
        	...
        	if(player.getInvincible() == 1) {
        		...
        		if(player.getDown() < getUp() + 10) {
        			...
        			if (getAnimationState().equals("normal") || getAnimationState().equals("kick")) {
